...
class TemporalUNet(nn.Module):
    ...
    def __init__(self, model_path, input_dim, time_dim, device, dims = (32, 64, 128, 256)):
    ...
    def forward(self, x, t):
        ...
        for i in range(len(self.down_samplers)):
            ...
            # print(f"Shape of h_list: {h.shape}")
            h_list.append(h)
        ...
        for i in range(len(self.up_samplers)):
            ...
            if(x.shape[2]!=h_temp.shape[2]):
                if x.shape[2] == 2 or x.shape[2] == 8 or x.shape[2] == 14 or x.shape[2] == 26 or x.shape[2] == 8: # Upsampling doubles the dimensions of the input. So, we are manually cropping the extra size to match the size of it's corresponding h (context/residual from the downsampling layer)
